...
def _compute_Zmatrix_from_u(u: np.ndarray) -> np.ndarray:
    ...
    -------
    np.ndarray
        Shape (n_freq, p, p, n_theta),
        interpreted as Z[freq, nu, j, theta]
    """
...
def compute_chunked_fft(
    ...
    diag_idx = np.arange(Y.shape[-1])
    Y[:, diag_idx, diag_idx] = Y[:, diag_idx, diag_idx].real
