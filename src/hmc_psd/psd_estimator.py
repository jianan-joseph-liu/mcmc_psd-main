...
class PSDEstimator:
    ...
    1. Optimize the learning rate to maximize the posterior in the MAP phase.
    2. Use HMC initialized from the MAP estimate to sample the posterior PSD.
    ...
    :ivar inference_runner: Object for running MAP and HMC inference.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param hmc_burnin_steps: Number of HMC warmup/burn-in steps, defaults to 500.
        :type hmc_burnin_steps: int, optional
        :param init_params: Optional external initial parameters assigned before MAP training.
        :type init_params: list, optional
        :param hmc_step_size: Initial leapfrog step size for HMC.
        :type hmc_step_size: float, optional
        :param hmc_num_leapfrog_steps: Leapfrog steps per HMC proposal.
    ...
    def _learning_rate_optimisation_objective(self, lr):
        ...
        Returns:
            float: Final negative HMC target log probability.
        """
        hmc_losses, _, _, _ = self.inference_runner.run(
            lr_map=lr["lr_map"],
            ntrain_map=self.ntrain_map,
            inference_size=self.N_samples,
            hmc_burnin_steps=self.hmc_burnin_steps,
    ...
    def _train_model(self):
        ...
        This method runs HMC to sample from the posterior PSD.
        """
    ...
    def plot_vi_losses(self) -> plt.Axes:
        """
        Plot the MAP and HMC target traces.
    ...
    def sample_posterior(self, n_samples: int = 1000) -> np.ndarray:
        ...
        :rtype: np.ndarray
        """
        if hasattr(self.inference_runner, "hmc_samples"):
            n_available = int(self.inference_runner.hmc_samples[0].shape[0])
            if n_samples > n_available:
                logger.warning(
                    f"Requested {n_samples} posterior samples, but only "
                    f"{n_available} HMC samples are available. Returning all."
                )
                n_samples = n_available
            spline_params = [
                sample[:n_samples] for sample in self.inference_runner.hmc_samples
            ]
        else:
